...
if [ -z "$LOCALIFY_KR_NAME" ]; then
  echo "No local APK found. Downloading the latest HoshimiLocalify APK from GitHub Releases..."
  gh release download --repo CyleAR/Hoshimi-localify --pattern "*.apk" --dir .
  LOCALIFY_KR_NAME=$(find . -maxdepth 1 -name 'HoshimiLocalify*.apk' -printf '%f\n' | head -n 1)
fi
